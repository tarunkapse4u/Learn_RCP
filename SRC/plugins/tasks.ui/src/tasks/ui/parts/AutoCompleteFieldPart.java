...
import tasks.ui.parts.contentassists.TaskContentProposal;
import tasks.ui.parts.contentassists.TaskContentProposalProvider;
...
public class AutoCompleteFieldPart
{
    ...
    public void createControls(final Composite parent)
    {
        ...
        Text text = WidgetFactory.text(SWT.BORDER).layoutData(GridDataFactory.fillDefaults().grab(true, false).create()).create(parent);
        TaskContentProposalProvider taskContentProposalProvider = new TaskContentProposalProvider(new ArrayList<>());
        ContentProposalAdapter contentProposal = new ContentProposalAdapter(text, new TextContentAdapter(), taskContentProposalProvider, null, null);
    ...
    private Path getPathWithoutFileName(final String inputPath)
    {
    ...
    private boolean separatorNotFound(final int lastIndex)
    {
        return lastIndex < 0;
    }

    private boolean endsWithSeparator(final String inputPath, final int lastIndex)
    {
        return lastIndex == inputPath.length();
    }

    private String removeFileName(final String text, final int lastIndex)
    {
        if (lastIndex == 0)
        {
    ...
    private Path getPath(final String text)
    {
        try
        {
    ...
    private boolean isDirectory(final Path dir)
    {
        try
        {
    ...
    private List<String> filterPaths(final Stream<Path> paths)
    {
