...
public class TodoDetailsPart
{
    ...
    public void createControls(final Composite parent)
    {
        ...
        txtDescription = WidgetFactory.text(SWT.BORDER | SWT.MULTI | SWT.V_SCROLL).layoutData(gdFactory.align(SWT.FILL, SWT.FILL).grab(true, true).create())
                .create(parent);
    ...
    @Inject
    public void setTasks(@Optional @Named(IServiceConstants.ACTIVE_SELECTION) final List<Task> tasks)
    {
    ...
    // if no task is set
    private void enableUserInterface(final boolean enabled)
    {
        if (txtSummary != null && !txtSummary.isDisposed())
        {
    ...
    private void updateUserInterface(final java.util.Optional<Task> task)
    {
        if (!task.isPresent())
        {
    ...
    @Persist // <.>
    public void save(final TaskService todoService)
    {
